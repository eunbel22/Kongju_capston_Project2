...
def expand_pronouns_with_history(user_input: str, conversation_history: list) -> str:
    """
    대명사를 이전 대화 맥락으로 확장 (조사 처리 개선)
    
    예시:
        "거기서 뭐 배워?" + 이전 대화(소프트웨어학과) → "소프트웨어학과에서 뭐 배워?"
        "거기 학과는?" + 이전 대화(천안캠퍼스) → "천안캠퍼스 학과는?"
        "거기에 도서관 있어?" + 이전 대화(예산캠퍼스) → "예산캠퍼스에 도서관 있어?"
    """
